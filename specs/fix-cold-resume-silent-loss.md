...
## 7. 冷启动运行时修复（Phase 9 — 本次修复）

> **背景**：Phase 5-8 已交付"本地优先 + 同步调度器 + guestBatchId 基础设施"，但实测冷启动
> 首次录音/发送仍有三条丢数据路径。本章节为 Phase 9 的修复范围。
>
> 三条真实丢失路径：
>   - **P1**：`gateway-client.send()` 在 `!getAccessToken()` 时静默 drop，冷启动 asr.start/asr.stop 丢包 → 录音"无识别、无提示"
>   - **P2**：`sync-orchestrator` 永久过滤 `userId === null` 条目；Phase 8 的 claim 被 feature flag 关闭 → 冷启动竞态产生的 userId=null capture 变僵尸
>   - **P3**：asr.done 永不到达时没有超时降级 → FAB 指令模式 & useVoiceToText 卡死无反馈

### 场景 7.1: gateway-client 未认证 / 未连接时，控制消息入待发队列（不再 drop）
```
假设 (Given)  gateway-client.send(msg) 被调用，且 getAccessToken() 为 null 或 WS 非 OPEN
当   (When)   消息类型为控制消息
那么 (Then)   按优先级入队 pendingControlFrames：
              - **必保留**（unboundedKeep）：chat.user, asr.start, asr.stop
              - **可丢弃**（bestEffort, 上限 50 FIFO）：asr.cancel, asr.partial-hint, heartbeat, read-receipt
并且 (And)    send() 立即返回（不抛错、不阻塞调用方）
并且 (And)    同 asr sessionId 组内消息"同生同灭"：
              - 若同 sessionId 出现 asr.cancel → 同 sessionId 的所有 bestEffort 帧立即清空
              - 若 asr.start 从必保留队列被强制移除（仅允许"发送成功 ack"触发） → 同 sessionId
                后续 asr.stop/cancel 也视为作废
              - 刷出时若 asr.start 发送失败 → 同 sessionId 后续帧全部回滚到队首，整组重试
并且 (And)    当 WS 进入 OPEN 且 getAccessToken() 非空后，按入队顺序串行刷出：
              - 带 client_id 的消息（chat.user / asr.start） → 收到 server 回显（chat.chunk/done
                或 asr.ack）才视为刷出成功，方可移出队列
              - 不带 client_id 的消息（asr.stop/cancel） → ws.send 无同步抛错即视为成功
              - ack 超过 10 秒未返回 → 保留在队首，标记 awaitingAck=true；不重复 send
              - 下次唤醒时若该消息仍在队列且 ws 仍 OPEN → 允许再次 send（由 gateway
                client_id 幂等兜底，不会重复生成）
并且 (And)    刷出过程中 WS 状态退化为 non-OPEN / 收到 401 → 立即中止本轮 flush，
              **禁止**继续调用 ws.send（浏览器会静默吞），已发出但未 ack 的消息保留 awaitingAck
并且 (And)    auth 层 refreshInFlight === true 期间 → 暂停 flush，等 refresh 完成再恢复
并且 (And)    二进制帧（sendBinary PCM）**不入队**：音频真相源是 audio_blobs，
              WS 未 OPEN 则丢弃 + console.warn（同现状）
并且 (And)    pendingControlFrames 不跨页面刷新持久化；真相源是 captureStore，
              刷新后由 sync-orchestrator 从 captureStore 重建控制消息
并且 (And)    多 tab：每 tab 独立 gateway-client + 独立队列；chat.user 幂等由 client_id
              跨 tab 去重（gateway 侧 §6 约束已保证）
```

### 场景 7.2: sync-orchestrator 对 userId=null 条目懒绑定当前用户
```
假设 (Given)  冷启动期间已产生 userId=null 的 capture，随后用户打开登录页
当   (When)   用户点击登录按钮并成功，auth:user-changed(kind="login") 触发 worker 扫描
那么 (Then)   worker 遍历 unsynced 队列，对每条 syncStatus ∈ {captured, failed} 且 userId === null 的 capture：
              读取 capture.guestBatchId 与当前 session 的 guestBatchId
并且 (And)    **只有** capture.guestBatchId === currentSessionBatchId **才进入懒绑定**：
              - 若不等（= 跨会话遗留） → 跳过，等 Phase 8.1 的 claim 流程显式处理
              - 若 capture.guestBatchId === null（不合法状态，Phase 1-7 僵尸）→ 跳过并告警
                console.warn("zombie capture without guestBatchId"); 等人工清理
并且 (And)    读取 auth 当前状态（getCurrentUser()）
并且 (And)    若 user 存在 → 在同一 IDB 事务中原子 update(localId, { userId: user.id, guestBatchId: null })
              后继续推送流程
并且 (And)    若 user 为 null → 保留 userId=null + guestBatchId 不变，等下次唤醒
并且 (And)    回填失败（IDB 异常） → 跳过本条，不影响后续条目
并且 (And)    **禁止** 对 syncStatus==="synced" 的条目做任何 userId 改写（防止事件回放污染历史）
并且 (And)    **guestBatchId 单进程锁**：同一设备多 tab 打开时，currentSessionBatchId
              由 localStorage key "voicenote:guestBatchId" 单例读取（不存在则原子创建），
              所有 tab 共享同一值；tab 关闭不清除；显式登出时由 Phase 8.1 管理
```

### 场景 7.3: ASR 完成超时降级提示
```
假设 (Given)  FAB 长按 / FAB 上滑指令 / ChatView useVoiceToText，用户已松手且 captureStore.create 已成功
当   (When)   asr.stop 发出后满足以下**任一**降级条件
              - 12 秒内未收到任何 asr.partial / asr.progress / asr.done / asr.error
              - 收到 asr.partial 但再等 8 秒无 asr.done / asr.error
那么 (Then)   UI 显示轻量 toast "录音已保存，转写将在联网后自动完成"（非阻塞 2 秒）
并且 (And)    本地 capture 保持 syncStatus="captured"，forceCommand 字段**保持原值**
              （指令意图不丢失；联网后 sync-orchestrator 仍按 forceCommand=true 推送给
              gateway 的指令路径）
并且 (And)    FAB / ChatView 录音状态机立即复位（允许用户开启下一次录音）
并且 (And)    迟到的 asr.done / asr.error 到达时——
              - 按 client_id（= localId）匹配 captureStore 条目 → 回写 transcript / serverId
              - **不再触碰 UI**（FAB 状态已复位，避免与新录音会话混淆）
              - 若 captureStore 中该 localId 已不存在（用户在超时后撤销删除） → 静默丢弃
并且 (And)    指令模式（forceCommand=true）超时 → toast 文案改为
              "指令已保存，将在联网后执行"；不退化为普通 diary，不在前端本地模拟执行
```

### 场景 7.4: 冷启动时序——auth 初始化期间的捕获可见性与归属隔离
```
假设 (Given)  App 刚启动，initAuth() 尚未完成或用户未登录
当   (When)   用户立即点击录音 / 发送
那么 (Then)   UI 不禁用任何输入控件（保持"永远响应"原则）
并且 (And)    captureStore.create 使用 currentSessionBatchId（场景 7.2 定义），userId=null
并且 (And)    列表渲染**按账号视图严格隔离**，防止跨账号泄漏：
              - 未登录视图 → 显示 userId===null ∧ guestBatchId===currentSessionBatchId
              - 登录视图（user=X） → 显示 userId===X **∪** (userId===null ∧ guestBatchId===currentSessionBatchId)
              - 其他账号 Y 的本地条目（userId===Y 或 guestBatchId≠currentSessionBatchId）**禁止**
                在 X 视图出现（包括音频回放 URL）
并且 (And)    initAuth() 完成且用户已登录 → 触发一次 auth:user-changed(kind="login") 事件
              → sync-orchestrator worker 走场景 7.2 懒绑定
并且 (And)    initAuth() 完成但未登录 → 条目保持 userId=null + guestBatchId 不变，UI 按
              "未登录视图"规则渲染
并且 (And)    **auth:user-changed 事件语义严格限定**：
              - 仅在"真实登录"（setLoggedInUser 被调用）/ "真实登出"（doLogout 被调用）触发
              - token silent refresh **不触发**该事件
              - 事件 payload 含 { kind: "login" | "logout", userId: string | null }
```

### 场景 7.5: 单一真相源 — userId 写入路径约束（防多路径冲突）
```
假设 (Given)  一条 capture 的 userId 字段可能被多处修改
当   (When)   实现冷启动相关代码
那么 (Then)   **userId 只允许由以下三条路径写入**，且互斥：
              1. captureStore.create 时初始写入（含 null）——之后创建点不再改写
              2. sync-orchestrator.worker 的懒绑定（场景 7.2，仅 captured/failed）
              3. guest-claim 流程的 claim（Phase 8.1，仅跨会话遗留）
并且 (And)    **禁止**：
              - 在 gateway-client.send 时从 token 推断 userId 回填 capture
              - 在 UI 组件（FAB / ChatView）内直接改 capture.userId
              - 任何路径改写 syncStatus==="synced" 条目的 userId
并且 (And)    Phase 8.1 的 claim 打开后，其 scope 必须排除 guestBatchId===currentSessionBatchId
              的条目（这些由 7.2 负责），只处理真正的跨会话遗留
```

### 场景 7.6: 回归保护——不允许的"修复"方式
```
假设 (Given)  未来开发者修改冷启动路径
当   (When)   引入以下任一做法
那么 (Then)   对应 regression 测试必须红：
              - ❌ 在 FAB / ChatView 入口处 await initAuth / await ensureFresh 阻塞 UI
              - ❌ capture 前 if (!userId) 提前 return
              - ❌ 弹窗"请先登录"阻塞录音/发送
              - ❌ gateway-client.send 扩大静默 drop 范围（扩大可丢弃白名单）
              - ❌ sync-orchestrator 对 userId=null 条目自动 hard delete
              - ❌ 列表渲染时把 userId≠currentUser 且 guestBatchId≠currentSessionBatchId
                的条目泄漏到当前账号视图
              - ❌ token silent refresh 触发 auth:user-changed
              - ❌ 在 syncStatus==="synced" 条目上改写 userId
并且 (And)    regression 测试 describe 标注 `regression: fix-cold-resume-silent-loss`，
              覆盖场景 7.1 / 7.2 / 7.3 / 7.4 跨账号隔离 / 7.5 单一写入路径 五类
并且 (And)    用户显式操作"清除所有本地数据"（设置页入口） **允许** hard delete
              包括 userId=null 条目——这是用户主动路径，不属于自动丢弃
```
...
- [ ] **deleteSync 对已同步条目的僵尸风险（P0.2 / C2 对抗审查补充）**：
      若 `CaptureRecord.serverId` 非空（ack 丢失但服务端其实入库成功），当前 gateway
      暂无按条删除的 `DELETE /api/v1/chat/messages/:id` 接口（仅 `DELETE /api/v1/chat/history`
      全量清空）。降级策略：前端仍本地删除 + 触发警告 toast
      "消息已从本设备删除，联网后将尝试清理服务端副本"。未来新增按条删除接口后，
      retrySync/deleteSync 应补齐服务端清理链路。
...
- [x] Phase 4: FAB 录音结束直接走 capture-store，不再 await WS（useVoiceToText 留待后续）
- [x] Phase 5: ChatView handleSend 改为本地先落地，乐观消息基于 syncStatus 展示
