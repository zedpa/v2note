...
# 提示词架构 v2 — 分层（SharedAgent / UserAgent / 五层存储 / endChat）

> 本文件为 `prompt-architecture-v2.md` 拆分后的「分层定义」子域。
> 另见 `prompt-architecture-v2-skills.md` — 自我维护工具 / Skill 重构 / System Prompt 组装 / Context Loader / 文件拆分 / 实施阶段。
...
```
假设 (Given)  需要存储用户自定义 Agent 配置
当   (When)   定义 user_agent 表结构
那么 (Then)   采用以下 schema：
```
...
```
假设 (Given)  新用户注册
当   (When)   系统初始化该用户的 Soul
那么 (Then)   从以下模板初始化（AI 随对话不断演化）
```
...
以下章节已拆分到 `prompt-architecture-v2-skills.md`：

- §5 自我维护工具定义（update_soul / update_profile / update_user_agent / create_memory / send_notification）
- §5b Skill 重构 — 全部默认关闭
