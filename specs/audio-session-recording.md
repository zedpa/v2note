...
```
假设 (Given)  后台正在播放音乐（Apple Music / QQ 音乐 / 网易云 等）
当   (When)   用户按住 FAB 开始录音
那么 (Then)   音乐播放被暂停
并且 (And)    录音正常开始，转写流程正常进行
```

### 场景 2: 录音正常结束恢复音频

```
假设 (Given)  录音进行中，系统音频已被打断
当   (When)   用户松开 FAB 或上滑结束录音
那么 (Then)   系统通知恢复，之前被打断的音频应用可自行恢复播放
```
...
```
假设 (Given)  录音进行中（音频会话已激活）
当   (When)   录音因网络失败 / 麦克风异常 / 识别超时 / Gateway 报错中断
那么 (Then)   系统音频恢复播放（不泄漏音频会话）
```

### 场景 5: 锁定模式全程打断

```
假设 (Given)  录音已右滑进入锁定模式
当   (When)   用户按暂停键
那么 (Then)   系统音频保持打断状态（不恢复）
并且 (And)    用户按停止键结束录音时系统音频恢复
并且 (And)    用户按取消（X）键时系统音频恢复
```

### 场景 7: Pre-capture 阶段即打断系统音频 <!-- ✅ completed (fix-recording-audio-focus) -->

```
假设 (Given)  后台正在播放音乐
当   (When)   用户长按 FAB 超过 120ms 触发 pre-capture
那么 (Then)   系统音频立即被打断
并且 (And)    麦克风随后正常打开，若用户短按取消则音频恢复播放
```
...
### 行为 1: 打断系统音频
- iOS/Android 后台播放音乐 → 按住录音 → 音乐停止 → 录音波形正常显示
