...
describe("ToolRegistry", () => {
    ...
    describe("注册与发现", () => {
        ...
        it("should_reject_duplicate_tool_names", () => {
            ...
            expect(() => registry.register(createMockTool({ name: "dup_tool" }))).toThrow(/already registered/);
        });
    });
    describe("工具执行", () => {
        ...
        it("should_return_error_for_unknown_tool", async () => {
            const ctx = { deviceId: "d1", sessionId: "s1" };
    ...
    describe("Vercel AI SDK 集成", () => {
        ...
        it("should_bind_context_to_ai_sdk_tool_execute", async () => {
            ...
            const ctx = { deviceId: "d1", userId: "u1", sessionId: "s1" };
            const aiTools = registry.toAISDKTools(ctx);
            // 模拟 AI SDK 调用 execute
            const executeFn = aiTools.my_tool.execute;
