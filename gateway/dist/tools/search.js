...
export async function unifiedSearch(params, ctx) {
    ...
    if (scopes.includes("memories"))
        promises.push(searchMemories(query, ctx, results));
    ...
    if (final.length > 0)
        console.log(`[search]   first result: ${JSON.stringify(final[0]).slice(0, 200)}`);
    return final;
}
// ── 日期解析 ────────────────────────────────────────────────────────────────
...
async function searchRecords(query, filters, ctx, results) {
    try {
        ...
        for (const r of records) {
            ...
            // domain 过滤
            if (filters.domain && r.domain !== filters.domain)
                continue;
...
// ── 记忆搜索 ────────────────────────────────────────────────────────────────
async function searchMemories(query, ctx, results) {
    try {
        if (!ctx.userId)
            return;
        ...
        for (const m of memories) {
            if (query && !m.content.toLowerCase().includes(queryLower))
                continue;
            results.push({
                id: m.id,
                type: "memory",
...
// ── AI 日报搜索 ──────────────────────────────────────────────────────────────
async function searchAiDiary(filters, ctx, results) {
    try {
        if (!ctx.userId)
            return;
        // 解析日期范围
        ...
        if (!dateFrom || !dateTo)
            return;
        const diaries = await aiDiaryRepo.findSummariesByUser(ctx.userId, "chat-daily", dateFrom, dateTo);
