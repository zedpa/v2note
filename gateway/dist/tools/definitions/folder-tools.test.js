...
describe("move_record", () => {
    ...
    it("should_move_record_to_new_domain_when_valid", async () => {
        ...
        const result = await moveRecordTool.handler({ record_id: "rec-1", domain: "工作/v2note" }, CTX);
        expect(result.success).toBe(true);
    ...
    it("should_move_to_uncategorized_when_domain_is_null", async () => {
        ...
        const result = await moveRecordTool.handler({ record_id: "rec-1", domain: null }, CTX);
        expect(result.success).toBe(true);
    ...
    it("should_fail_when_record_not_found", async () => {
        ...
        const result = await moveRecordTool.handler({ record_id: "nonexistent", domain: "工作" }, CTX);
    ...
    it("should_fail_when_no_access", async () => {
        ...
        const result = await moveRecordTool.handler({ record_id: "rec-2", domain: "工作" }, CTX);
        expect(result.success).toBe(false);
    });
...
describe("manage_folder", () => {
    ...
    it("should_create_folder_when_name_not_exists", async () => {
        ...
        const result = await manageFolderTool.handler({ action: "create", name: "旅行" }, CTX);
        expect(result.success).toBe(true);
    ...
    it("should_fail_create_when_no_name", async () => {
        const result = await manageFolderTool.handler({ action: "create" }, CTX);
        expect(result.success).toBe(false);
    ...
    it("should_rename_folder_and_update_records", async () => {
        ...
        const result = await manageFolderTool.handler({ action: "rename", old_name: "杂项", new_name: "其他" }, CTX);
        expect(result.success).toBe(true);
    ...
    it("should_fail_rename_when_missing_params", async () => {
        const result = await manageFolderTool.handler({ action: "rename", old_name: "杂项" }, CTX);
        expect(result.success).toBe(false);
    });
    // ── delete ──
    it("should_delete_folder_and_clear_domain", async () => {
        vi.mocked(recordRepo.clearDomainByPrefix).mockResolvedValue(8);
        const result = await manageFolderTool.handler({ action: "delete", name: "杂项" }, CTX);
        expect(result.success).toBe(true);
    ...
    it("should_fail_delete_when_no_name", async () => {
        const result = await manageFolderTool.handler({ action: "delete" }, CTX);
        expect(result.success).toBe(false);
    });
    // ── merge ──
    it("should_merge_source_into_target", async () => {
        vi.mocked(recordRepo.batchUpdateDomain).mockResolvedValue(5);
        const result = await manageFolderTool.handler({ action: "merge", source: "工作/杂项", target: "工作" }, CTX);
        expect(result.success).toBe(true);
    ...
    it("should_fail_merge_when_missing_params", async () => {
        const result = await manageFolderTool.handler({ action: "merge", source: "工作/杂项" }, CTX);
        expect(result.success).toBe(false);
    });
...
//# sourceMappingURL=folder-tools.test.js.map