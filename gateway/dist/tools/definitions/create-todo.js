...
不用：用户要求批量创建多条 → 交给 Plan 机制。`,
    ...
    handler: async (args, ctx) => {
        ...
        if (schedule.scheduled_start !== undefined)
            updates.scheduled_start = schedule.scheduled_start || null;
        if (schedule.scheduled_end !== undefined)
            updates.scheduled_end = schedule.scheduled_end || null;
        if (schedule.estimated_minutes !== undefined)
            updates.estimated_minutes = schedule.estimated_minutes;
        if (schedule.priority !== undefined)
            updates.priority = schedule.priority;
