...
export const updateUserInfoTool = {
...
不用：只需要查看画像 → 用 view(type:"profile")。`,
    ...
    handler: async (args, ctx) => {
        ...
        const fields = ["name", "occupation", "current_focus", "pain_points", "review_time"];
        ...
        for (const field of fields) {
            if (args[field] !== undefined) {
                await userProfileRepo.upsertOnboardingField(ctx.userId, field, args[field], ctx.deviceId);
