...
import { PROCESS_GUARDRAILS, CHAT_GUARDRAILS, BRIEFING_GUARDRAILS, } from "../context/anti-hallucination.js";
const __dirname = dirname(fileURLToPath(import.meta.url));
// Load Agent.md once at startup — split into core rules vs full content
let agentMdFull;
let agentMdCore; // compact version for hot tier
try {
...
export function buildTieredContext(opts) {
    ...
    if (opts.skills.length > 0) {
        const { fullText, metadataOnly } = partitionSkillsByRelevance(opts.skills, opts.inputText);
        ...
        if (metadataOnly.length > 0) {
            cold.push(`以下技能已加载但未展开详情（当前输入可能不相关）：\n` +
