...
export function getPerspective(dayOfWeek) {
...
export async function generateMorningReport(deviceId, userId) {
    ...
    const [soul, profile, pendingTodos, activeGoals, yesterdayStats, streak] = await Promise.all([
        ...
        (userId
            ...
            : todoRepo.countByDateRange(deviceId, `${yesterday}T00:00:00Z`, `${yesterday}T23:59:59Z`)).catch(() => ({ done: 0, total: 0 })),
    ...
    const goalsText = activeGoals.length > 0
        ? activeGoals.slice(0, 5).map((g) => `- ${g.title} (${g.status})`).join("\n")
        : "暂无活跃目标";
...
export async function generateEveningReport(deviceId, userId) {
    ...
    try {
        ...
        const todayOnly = records.filter((r) => r.created_at && toDateString(r.created_at)?.startsWith(today));
        newRecordCount = todayOnly.length;
        todayRecords = todayOnly.slice(0, 5).map((r) => ({
    ...
    const goalsText = activeGoals.slice(0, 5).map((g) => `- ${g.title} (${g.status})`).join("\n") || "无";
    // 渲染 prompt
