...
export function toLocalDateStr(v) {
    if (!v)
        return null;
...
// ── UserAgent 通知偏好检查 ──
export async function isBriefingDisabled(userId, type) {
    if (!userId)
        return false;
    try {
        ...
        for (const line of lines) {
            if (line.includes(type) && line.includes("关闭"))
                return true;
        }
        return false;
    }
...
${goalPulseText}`,
    ...
    try {
        ...
        try {
            await briefingRepo.upsert(deviceId, today, "morning", fallback, userId);
        }
        catch { /* ignore */ }
        return fallback;
    }
...
export async function generateEveningSummary(deviceId, userId, forceRefresh) {
    ...
    const pending = userId
        ? await todoRepo.findPendingByUser(userId)
        : await todoRepo.findPendingByDevice(deviceId);
    const tomorrowScheduled = pending.filter((t) => toLocalDateStr(t.scheduled_start) === tomorrow);
    ...
    if (userId) {
        try {
            ...
            if (records.length > 0) {
                const transcripts = await transcriptRepo.findByRecordIds(records.map((r) => r.id));
                // 按完整 record 边界截断到 2000 字（至少保留第一条）
                let charCount = 0;
                const parts = [];
                for (const t of transcripts) {
                    if (parts.length > 0 && charCount + t.text.length > 2000)
                        break;
...
待处理(${pending.length}): ${pending.slice(0, 5).map((t) => t.text).join("、") || "无"}${diaryBlock}`,
    ...
    try {
        ...
        // Cache
        try {
            await briefingRepo.upsert(deviceId, today, "evening", parsed, userId);
        }
        catch { /* ignore */ }
        // 异步生成聊天日记（不阻塞 evening summary 返回）
        if (userId) {
            generateChatDiary(deviceId, userId, today).catch(e => console.warn(`[daily-loop] Chat diary failed: ${e.message}`));
        }
        ...
        try {
            await briefingRepo.upsert(deviceId, today, "evening", fallback, userId);
        }
        catch { /* ignore */ }
        return fallback;
    }
