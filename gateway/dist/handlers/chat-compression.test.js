...
import { shouldCompress, compressMessages, COMPRESS_PROMPT, } from "./chat-compression.js";
...
describe("chat-compression", () => {
    ...
    describe("compressMessages", () => {
        it("should_compress_oldest_messages_keeping_recent_20", async () => {
            ...
            vi.mocked(getUncompressedMessages).mockResolvedValue(msgs);
            ...
            // 应该压缩前 30 条（50 - 20 = 30）
            expect(markCompressed).toHaveBeenCalledWith(msgs.slice(0, 30).map(m => m.id));
            // 应该保存 context-summary
            expect(saveMessage).toHaveBeenCalledWith("u-1", "context-summary", "这是压缩摘要");
        });
        it("should_use_background_tier_for_compression", async () => {
            ...
            vi.mocked(getContextSummaries).mockResolvedValue([]);
            await compressMessages("u-1");
            expect(chatCompletion).toHaveBeenCalledWith(expect.any(Array), expect.objectContaining({ tier: "background" }));
        });
    });
    // ── 场景 4.3: 压缩 prompt ──
...
//# sourceMappingURL=chat-compression.test.js.map