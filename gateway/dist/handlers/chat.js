...
export async function startChat(payload) {
    ...
    if (payload.mode === "decision") {
        ...
        if (isGreeting) {
            ...
            const greetingPrompt = await buildGreetingPrompt(payload.userId, payload.deviceId, transcriptSummary);
    ...
    try {
        ...
        if (!report.is_empty) {
            ...
            if (report.contradictions.length > 0) {
                const cList = report.contradictions.slice(0, 3).map(c => `「${c.strikeA_nucleus.slice(0, 30)}」↔「${c.strikeB_nucleus.slice(0, 30)}」`);
...
async function* streamWithNativeTools(session, deviceId, tierOverride) {
    ...
    const stream = streamWithTools(messages, aiTools, { temperature: 0.7, maxSteps: 5, tier });
    let fullResponse = "";
    for await (const chunk of stream) {
        fullResponse += chunk;
        yield chunk;
