...
describe("iterateStreamWithTimeout — stream 超时保护", () => {
    ...
    it("should_timeout_when_stream_hangs", async () => {
        ...
        const chunks = [];
        // 使用短超时测试
        await expect(iterateStreamWithTimeout(hangingStream(), (chunk) => {
            chunks.push(chunk);
        }, 100)).rejects.toThrow("Stream timeout");
    ...
    it("should_propagate_stream_errors", async () => {
        ...
        const chunks = [];
        await expect(iterateStreamWithTimeout(errorStream(), (chunk) => {
            chunks.push(chunk);
...
//# sourceMappingURL=chat-stream-safety.test.js.map