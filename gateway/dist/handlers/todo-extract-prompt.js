...
export function buildTodoExtractPrompt(ctx) {
    const dateAnchor = buildDateAnchor();
    const pendingList = ctx.pendingTodos.length > 0
        ? ctx.pendingTodos.map((t, i) => `  ${i + 1}. [${t.id}] "${t.text}"${t.scheduled_start ? ` (${t.scheduled_start})` : ""}`).join("\n")
        : "  （无未完成待办）";
 ...
 */
export function buildTodoRefinePrompt(currentCommands, dateAnchor) {
...
//# sourceMappingURL=todo-extract-prompt.js.map