 ...
 */
export async function computeRecordRelations(recordId, strikes, crossBonds, targetStrikeCounts, strikeSourceTypes, strikeToRecord) {
    const myStrikeIds = new Set(strikes.map((s) => s.id));
    const myStrikeCount = strikes.length;
    if (myStrikeCount === 0 || crossBonds.length === 0)
        return [];
    // 按目标日记分组累加 bond strength
    const recordStrengths = new Map();
    for (const bond of crossBonds) {
        ...
        recordStrengths.set(targetRecordId, (recordStrengths.get(targetRecordId) ?? 0) + strength);
    }
    // 计算关联度
    const results = [];
...
//# sourceMappingURL=record-relations.js.map