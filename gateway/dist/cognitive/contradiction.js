...
async function bondExists(idA, idB) {
    const rows = await query(`SELECT id FROM bond
...
async function classifyPairs(pairs) {
...
// Main entry
// ---------------------------------------------------------------------------
export async function scanContradictions(userId, daysBack) {
    const days = daysBack ?? 7;
    // Step 1: fetch recent Judge/Perceive strikes
    const recentStrikes = await query(`SELECT * FROM strike
    ...
    const allPairs = [];
    const seen = new Set(); // "idA|idB" dedup within this scan
    for (const strike of recentStrikes) {
        ...
        for (const r of results) {
            ...
            if (r.strike.polarity === strike.polarity)
                continue;
            if (r.score <= 0.5)
                continue;
            if (r.strike.id === strike.id)
                continue;
    ...
    for (let i = 0; i < allPairs.length; i += 10) {
        ...
        for (const v of verdicts) {
            if (v.pair_idx < 0 || v.pair_idx >= batch.length)
                continue;
