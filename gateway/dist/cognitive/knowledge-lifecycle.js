 ...
 * 基于 embedding 相似度检测。
 */
export async function scanExpiredFacts(userId) {
    const rows = await query(`SELECT old_s.id as old_id, old_s.nucleus as old_nucleus,
 ...
 */
export async function getSupersedAlerts(userId) {
    const rows = await query(`SELECT s.id, s.nucleus, s.superseded_by,
 ...
 */
export async function undoSupersede(strikeId) {
    await execute(`UPDATE strike SET status = 'active', superseded_by = NULL, updated_at = NOW() WHERE id = $1`, [strikeId]);
}
