...
vi.mock("../handlers/digest.js", () => ({
    digestRecords: (...args) => mockDigestRecords(...args),
}));
import { detectCognitiveQuery, loadChatCognitive, buildGoalDiscussionContext, buildInsightDiscussionContext, formatCitation, saveConversationAsRecord, } from "./advisor-context.js";
beforeEach(() => {
    vi.resetAllMocks();
...
describe("saveConversationAsRecord", () => {
    it("should_create_record_with_source_type_think", async () => {
        ...
        // transcript 应存储实际文本
        expect(mockTranscriptRepo.create).toHaveBeenCalledWith(expect.objectContaining({
            record_id: "rec-new-1",
    ...
    it("should_trigger_digest_pipeline", async () => {
        ...
        expect(mockDigestRecords).toHaveBeenCalledWith(["rec-new-1"], { deviceId: "device-1", userId: "user-1" });
    });
...
//# sourceMappingURL=advisor-context.test.js.map