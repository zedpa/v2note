 ...
 * @returns 编译结果
 */
export async function compileWikiForUser(userId, maxRecords = 30) {
    const emptyResult = {
        pages_created: 0,
        pages_updated: 0,
        pages_split: 0,
    ...
    try {
        ...
        for (let batch = 0; batch < MAX_BATCH_ITERATIONS; batch++) {
            ...
            const allPageIndex = allPages.map((p) => ({
                id: p.id,
                title: p.title,
                summary: p.summary,
                level: p.level,
            ...
            const existingDomains = [...new Set(allPages.map((p) => p.domain).filter(Boolean))];
            // ── 阶段 B: 编译（AI 调用）──
            const promptInput = {
            ...
            const batchResult = await executeInstructions(instructions, userId, pendingRecords.map((r) => r.id));
            ...
            void updatePageEmbeddings(instructions, userId).catch((err) => console.warn(`[wiki-compiler] embedding 更新失败: ${err.message}`));
            tBatch(`done`);
            // 本轮处理的 record 少于 maxRecords，说明已全部处理完
            if (pendingRecords.length < maxRecords)
                break;
        }
        ...
        if (emptyResult.records_compiled > 0) {
            const summaryParts = [];
            if (emptyResult.pages_created > 0)
                summaryParts.push(`新建 ${emptyResult.pages_created} 个 page`);
            if (emptyResult.pages_updated > 0)
                summaryParts.push(`更新 ${emptyResult.pages_updated} 个 page`);
            if (emptyResult.pages_split > 0)
                summaryParts.push(`拆分 ${emptyResult.pages_split} 个 page`);
            if (emptyResult.pages_merged > 0)
                summaryParts.push(`合并 ${emptyResult.pages_merged} 个 page`);
...
async function loadRecordTexts(records) {
    ...
    const transcripts = await query(`SELECT record_id, text FROM transcript WHERE record_id IN (${placeholders})`, ids);
    const summaries = await query(`SELECT record_id, short_summary FROM summary WHERE record_id IN (${placeholders})`, ids);
    // 查询 embedding
    const embeddings = await query(`SELECT id, embedding FROM record WHERE id IN (${placeholders}) AND embedding IS NOT NULL`, ids);
...
async function routeRecordsToPages(records, pages) {
    const matchedMap = new Map();
    for (const record of records) {
        if (!record.embedding || pages.length === 0)
            continue;
        const scored = [];
        ...
        for (const s of scored.slice(0, MAX_MATCHED_PAGES)) {
            ...
            if (s.similarity > existing)
                matchedMap.set(s.pageId, s.similarity);
        }
    }
    return matchedMap;
}
/** 加载命中 page 的完整 content，按相似度排序，受 token 预算限制 */
async function loadMatchedPages(matchedMap, allPages) {
    ...
    for (const [pageId] of sorted) {
        ...
        if (totalChars + contentLen > MAX_CONTENT_CHARS)
            break;
...
async function callAIWithRetry(system, user, promptInput, recordsWithText) {
    try {
...
/** 解析 AI 返回的 JSON */
export function parseCompileResponse(raw) {
...
export async function executeInstructions(instructions, userId, recordIds) {
    const pool = getPool();
    ...
    try {
        ...
        for (const upd of instructions.update_pages) {
            ...
            await client.query(`UPDATE wiki_page SET content = $1, summary = $2, compiled_at = now(), updated_at = now() WHERE id = $3`, [upd.new_content, upd.new_summary, upd.page_id]);
            for (const recId of upd.add_record_ids) {
                if (!isValidUuid(recId))
                    continue;
                await client.query(`INSERT INTO wiki_page_record (wiki_page_id, record_id)
           SELECT $1, $2 WHERE EXISTS (SELECT 1 FROM record WHERE id = $2)
           ON CONFLICT DO NOTHING`, [upd.page_id, recId]);
            }
        ...
        for (const cp of instructions.create_pages) {
            ...
            const createResult = await client.query(`INSERT INTO wiki_page (user_id, title, content, summary, parent_id, level, domain, metadata)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`, [
            ...
            for (const recId of cp.record_ids) {
                ...
                await client.query(`INSERT INTO wiki_page_record (wiki_page_id, record_id)
           SELECT $1, $2 WHERE EXISTS (SELECT 1 FROM record WHERE id = $2)
           ON CONFLICT DO NOTHING`, [newPageId, recId]);
            }
        ...
        for (const sp of instructions.split_page) {
            ...
            // 更新原 page 的 content
            await client.query(`UPDATE wiki_page SET content = $1, compiled_at = now(), updated_at = now() WHERE id = $2`, [sp.new_parent_content, sp.source_id]);
            // 创建子 page
            for (const child of sp.children) {
                const childResult = await client.query(`INSERT INTO wiki_page (user_id, title, content, summary, parent_id, level, domain, metadata)
           VALUES ($1, $2, $3, $4, $5,
             ...
             ON CONFLICT DO NOTHING`, [childId, sp.source_id]);
                }
            }
            result.pages_split++;
        }
        ...
        for (const mp of instructions.merge_pages) {
            ...
            // source 标记为 merged
            await client.query(`UPDATE wiki_page SET status = 'merged', merged_into = $1, updated_at = now() WHERE id = $2`, [mp.target_id, mp.source_id]);
            // 迁移 record 关联（先删除重复再迁移，避免 PK 冲突）
            await client.query(`DELETE FROM wiki_page_record WHERE wiki_page_id = $1
         AND record_id IN (SELECT record_id FROM wiki_page_record WHERE wiki_page_id = $2)`, [mp.source_id, mp.target_id]);
            await client.query(`UPDATE wiki_page_record SET wiki_page_id = $1 WHERE wiki_page_id = $2`, [mp.target_id, mp.source_id]);
            // 迁移 goal 关联
            await client.query(`UPDATE todo SET wiki_page_id = $1 WHERE wiki_page_id = $2 AND level >= 1`, [mp.target_id, mp.source_id]);
            result.pages_merged++;
        }
        // 5. goal_sync
        for (const gs of instructions.goal_sync) {
           ...
           VALUES ($1, $2, $3, $4, 1, false, 'emerged', $5)`, [deviceId, userId, gs.title, gs.status ?? "active", goalPageId]);
                ...
                if (params.length > 0) {
                    ...
                    await client.query(`UPDATE todo SET ${sets.join(", ")} WHERE id = $${i} AND level >= 1`, params);
                }
            }
        }
        // 6. 所有 Record 标记为 compiled
        if (recordIds.length > 0) {
            await client.query(`UPDATE record SET compile_status = 'compiled', updated_at = now() WHERE id = ANY($1)`, [recordIds]);
        ...
        if (result.pages_created > 0)
            summaryParts.push(`新建 ${result.pages_created} 个 page`);
        if (result.pages_updated > 0)
            summaryParts.push(`更新 ${result.pages_updated} 个 page`);
        if (result.pages_split > 0)
            summaryParts.push(`拆分 ${result.pages_split} 个 page`);
        if (result.pages_merged > 0)
            summaryParts.push(`合并 ${result.pages_merged} 个 page`);
        if (summaryParts.length > 0) {
...
async function updatePageEmbeddings(instructions, _userId) {
    const pageIdsToUpdate = new Set();
    for (const upd of instructions.update_pages) {
