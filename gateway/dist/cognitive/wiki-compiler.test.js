...
vi.mock("../db/repositories/record.js", () => ({
    findPendingCompile: (...args) => mockFindPendingCompile(...args),
    updateCompileStatus: (...args) => mockUpdateCompileStatus(...args),
}));
vi.mock("../db/repositories/wiki-page.js", () => ({
    findByUser: (...args) => mockWikiPageFindByUser(...args),
    findById: (...args) => mockWikiPageFindById(...args),
    create: (...args) => mockWikiPageCreate(...args),
    update: (...args) => mockWikiPageUpdate(...args),
    updateStatus: (...args) => mockWikiPageUpdateStatus(...args),
    findByParent: (...args) => mockWikiPageFindByParent(...args),
}));
...
vi.mock("./wiki-compile-prompt.js", () => ({
    buildCompilePrompt: (...args) => mockBuildCompilePrompt(...args),
}));
vi.mock("../lib/tz.js", () => ({
    today: () => "2026-04-09",
    now: () => new Date("2026-04-09T10:00:00+08:00"),
}));
import { compileWikiForUser, parseCompileResponse, cosineSimilarity, executeInstructions, } from "./wiki-compiler.js";
describe("wiki-compiler", () => {
    ...
    describe("cold_start_compilation", () => {
        it("should_set_cold_start_flag_when_no_wiki_pages", async () => {
            ...
            expect(mockBuildCompilePrompt).toHaveBeenCalledWith(expect.objectContaining({ isColdStart: true }));
    ...
    describe("db_transaction_rollback", () => {
        it("should_rollback_all_changes_when_instruction_execution_fails", async () => {
            ...
            await expect(executeInstructions(instructions, "user-1", ["rec-1"])).rejects.toThrow("DB write failed");
            // 验证 ROLLBACK 被调用
    ...
    describe("goal_sync_create", () => {
        it("should_create_goal_with_wiki_page_id_when_goal_sync_create", async () => {
            ...
            const insertCall = mockClient.query.mock.calls.find((call) => typeof call[0] === "string" && call[0].includes("INSERT INTO todo"));
    ...
    describe("page_split", () => {
        it("should_create_child_pages_and_update_parent_when_split", async () => {
            ...
            // 验证父 page 被更新
            const updateCall = mockClient.query.mock.calls.find((call) => typeof call[0] === "string" && call[0].includes("UPDATE wiki_page SET content"));
            expect(updateCall).toBeDefined();
            // 验证子 page 被创建（2个子 page）
            const insertCalls = mockClient.query.mock.calls.filter((call) => typeof call[0] === "string" && call[0].includes("INSERT INTO wiki_page ("));
            expect(insertCalls.length).toBe(2);
            // 验证 record 关联迁移到子 page（每个子 page 一条 INSERT INTO wiki_page_record）
            const recordMigrateCalls = mockClient.query.mock.calls.filter((call) => typeof call[0] === "string" && call[0].includes("INSERT INTO wiki_page_record"));
    ...
    describe("merge_pages", () => {
        it("should_mark_source_as_merged_and_migrate_records", async () => {
            ...
            const statusUpdateCall = mockClient.query.mock.calls.find((call) => typeof call[0] === "string" &&
                call[0].includes("status = 'merged'") &&
                call[0].includes("merged_into"));
            expect(statusUpdateCall).toBeDefined();
            // 验证 record 关联迁移
            const recordMigrateCall = mockClient.query.mock.calls.find((call) => typeof call[0] === "string" &&
                call[0].includes("UPDATE wiki_page_record"));
            expect(recordMigrateCall).toBeDefined();
            // 验证 goal 关联迁移
            const goalMigrateCall = mockClient.query.mock.calls.find((call) => typeof call[0] === "string" &&
                call[0].includes("UPDATE todo SET wiki_page_id"));
            expect(goalMigrateCall).toBeDefined();
        });
    });
});
//# sourceMappingURL=wiki-compiler.test.js.map