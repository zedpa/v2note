...
function buildSystemPrompt(dateAnchor, existingDomains, isColdStart) {
...
function buildUserMessage(newRecords, matchedPages, allPageIndex) {
    const parts = [];
    // 新 Record 列表
    ...
    if (allPageIndex.length > 0) {
        ...
        for (const page of allPageIndex) {
            parts.push(`| ${page.id} | ${page.title} | ${page.summary ?? "-"} | L${page.level} | ${page.domain ?? "-"} |`);
        }
    }
    parts.push("\n请根据以上信息输出 JSON 编译指令。");
    return parts.join("\n");
}
//# sourceMappingURL=wiki-compile-prompt.js.map