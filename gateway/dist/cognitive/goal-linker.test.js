...
// ── Import after mocks ────────────────────────────────────────────────
const { computeGoalHealth, checkIntendEmergence, updateGoalStatus, createActionEvent, getGoalTimeline, } = await import("./goal-linker.js");
// ── Tests ─────────────────────────────────────────────────────────────
beforeEach(() => {
    vi.resetAllMocks();
    // 恢复默认实现
...
describe("场景 13: intend 密度超标触发目标涌现", () => {
    it("should_suggest_goal_when_intend_density_exceeds_threshold", async () => {
        ...
        mockGoalCreate.mockResolvedValue(makeGoal({ id: "g-new", title: "供应链管理", status: "active" }));
...
describe("场景 9-10: 行动事件持久化", () => {
    it("should_create_skip_event", async () => {
        ...
        const sql2 = mockExecute.mock.calls[1][0];
...
describe("场景 15: 目标状态流转", () => {
    it("should_transition_active_to_progressing_when_todo_completed", async () => {
        const goalId = "goal-1";
        mockGoalFindById.mockResolvedValue(makeGoal({ id: goalId, status: "active" }));
        mockGoalFindWithTodos.mockResolvedValue([
            { id: "t1", done: true },
            { id: "t2", done: false },
        ]);
    ...
    it("should_transition_progressing_to_blocked_when_skip_threshold", async () => {
        const goalId = "goal-2";
        mockGoalFindById.mockResolvedValue(makeGoal({ id: goalId, status: "progressing" }));
        // 有 todo 被跳过 3+ 次
    ...
    it("should_not_transition_when_already_archived", async () => {
        const goalId = "goal-3";
        mockGoalFindById.mockResolvedValue(makeGoal({ id: goalId, status: "abandoned" }));
        await updateGoalStatus(goalId, "todo_completed");
...
describe("场景 16: 目标时间线", () => {
    it("should_return_related_records_via_cluster", async () => {
        ...
        mockGoalFindById.mockResolvedValueOnce(makeGoal({ id: goalId, cluster_id: "cluster-1" }));
        // 通过 cluster 成员追溯日记
        mockQuery.mockResolvedValueOnce([
...
//# sourceMappingURL=goal-linker.test.js.map