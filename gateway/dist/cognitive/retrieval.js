...
async function semanticChannel(nucleus, userId, topK) {
    try {
        const embedding = await getEmbedding(nucleus);
        const pgVector = `[${embedding.join(",")}]`;
        const rows = await query(`SELECT *, 1 - (embedding <=> $1::vector) as similarity
       FROM strike
       WHERE user_id = $2 AND status = 'active'
         AND is_cluster = false AND embedding IS NOT NULL
       ORDER BY embedding <=> $1::vector
       LIMIT $3`, [pgVector, userId, topK]);
...
async function polarityChannel(nucleus, polarity, userId, limit) {
    if (!polarity)
        return [];
    try {
        const embedding = await getEmbedding(nucleus);
       ...
       LIMIT $4`, [userId, polarity, pgVector, limit]);
    }
    catch (err) {
...
async function clusterChannel(nucleus, userId, topClusters, membersPerCluster) {
    try {
       ...
       ORDER BY embedding <=> $1::vector
       LIMIT $3`, [pgVector, userId, topClusters]);
        ...
        for (const cluster of clusters) {
         ...
         LIMIT $3`, [pgVector, cluster.id, membersPerCluster]);
...
export async function hybridRetrieve(nucleus, tags, userId, opts) {
    ...
    const [semanticResults, tagResults, personResults, temporalResults, polarityResults, clusterResults] = await Promise.all([
