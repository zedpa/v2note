...
export async function hybridRetrieve(nucleus, tags, userId, opts) {
    ...
    const [semanticResults, tagPersonResults, temporalResults, polarityResults, clusterResults] = await Promise.all([
        ...
        pgVector
            ? query(`SELECT *, 1 - (embedding <=> $1::vector) as similarity
             ...
             LIMIT $3`, [pgVector, userId, 5]).then((rows) => rows.map((r) => ({ strike: r, similarity: r.similarity }))).catch((err) => {
        ...
        allLabels.length > 0
            ? (() => {
                ...
                return query(`SELECT DISTINCT s.*, st.label FROM strike s
               JOIN strike_tag st ON st.strike_id = s.id
               WHERE s.user_id = $1 AND s.status = 'active'
                 AND st.label IN (${placeholders})
               ORDER BY s.created_at DESC
               LIMIT $${allLabels.length + 2}`, [userId, ...allLabels, 6]).catch((err) => {
        ...
        pgVector && opts?.polarity
            ? query(`SELECT * FROM strike
             WHERE user_id = $1 AND status = 'active'
               AND polarity != $2 AND embedding IS NOT NULL
               AND 1 - (embedding <=> $3::vector) > 0.7
             ORDER BY embedding <=> $3::vector
             LIMIT $4`, [userId, opts.polarity, pgVector, 3]).catch((err) => {
        ...
        pgVector
            ? (async () => {
                try {
                    const clusters = await query(`SELECT *, 1 - (embedding <=> $1::vector) as similarity
                 FROM strike
                 WHERE user_id = $2 AND is_cluster = true AND status = 'active'
                   AND embedding IS NOT NULL
                 ORDER BY embedding <=> $1::vector
                 LIMIT $3`, [pgVector, userId, 3]);
                    if (clusters.length === 0)
                        return [];
                    // 批量加载所有 cluster 的成员（一条查询替代 N 条）
                    const clusterIds = clusters.map((c) => c.id);
                    const memberPlaceholders = clusterIds.map((_, i) => `$${i + 2}`).join(", ");
                    const members = await query(`SELECT s.*, 1 - (s.embedding <=> $1::vector) as similarity
                 FROM bond b
                 ...
                 LIMIT $${clusterIds.length + 2}`, [pgVector, ...clusterIds, 9]);
