...
async function loadGraph(userId) {
    ...
    const bonds = await query(`SELECT * FROM bond
     WHERE source_strike_id IN (${ph})
       AND target_strike_id IN (${ph})`, [...ids, ...ids]);
...
// ---------------------------------------------------------------------------
function computeClosureScores(adj) {
...
// ---------------------------------------------------------------------------
function discoverCandidates(adj, closureScores) {
    ...
    for (const seed of seeds) {
        ...
        while (queue.length > 0) {
            ...
            if (inCluster.has(cur) || visited.has(cur))
                continue;
            // BFS expansion: require closure > 0.2 (seed always qualifies)
            if (inCluster.size > 0 && (closureScores.get(cur) ?? 0) <= 0.2)
                continue;
            inCluster.add(cur);
            visited.add(cur);
            cluster.push(cur);
            for (const nb of adj.get(cur) ?? []) {
                if (!visited.has(nb) && !inCluster.has(nb))
                    queue.push(nb);
            }
        }
        if (cluster.length >= 3) {
...
async function filterExisting(candidates) {
    ...
    for (const memberIds of candidates) {
        ...
        const rows = await query(`SELECT source_strike_id AS cluster_strike_id,
              target_strike_id AS member_strike_id
       FROM bond
       WHERE type = 'cluster_member'
         AND target_strike_id IN (${ph})`, memberIds);
        // Group by cluster
        const byCluster = new Map();
        for (const r of rows) {
            if (!byCluster.has(r.cluster_strike_id))
                byCluster.set(r.cluster_strike_id, new Set());
...
// Step 5: AI review
// ---------------------------------------------------------------------------
async function aiReview(strikes) {
    const list = strikes
...
// ---------------------------------------------------------------------------
async function createCluster(userId, verdict, memberIds) {
...
async function addMembersToCluster(clusterId, newMemberIds) {
    // Only add members not already present
    const existing = await query(`SELECT target_strike_id FROM bond
     WHERE source_strike_id = $1 AND type = 'cluster_member'`, [clusterId]);
    const existingSet = new Set(existing.map((r) => r.target_strike_id));
    const toAdd = newMemberIds.filter((id) => !existingSet.has(id));
    if (toAdd.length > 0) {
        await bondRepo.createMany(toAdd.map((id) => ({
...
async function buildInterClusterBonds(clusterIds) {
    if (clusterIds.length < 2)
        return;
    for (let i = 0; i < clusterIds.length; i++) {
        const membersA = await query(`SELECT target_strike_id FROM bond
       WHERE source_strike_id = $1 AND type = 'cluster_member'`, [clusterIds[i]]);
        const setA = new Set(membersA.map((r) => r.target_strike_id));
        for (let j = i + 1; j < clusterIds.length; j++) {
            const membersB = await query(`SELECT target_strike_id FROM bond
         WHERE source_strike_id = $1 AND type = 'cluster_member'`, [clusterIds[j]]);
            // Check if any member of B has a bond with any member of A
            const bIds = membersB.map((r) => r.target_strike_id);
            if (bIds.length === 0 || setA.size === 0)
                continue;
            const ph = bIds.map((_, k) => `$${k + 1}`).join(", ");
            const aArr = [...setA];
            const phA = aArr.map((_, k) => `$${k + bIds.length + 1}`).join(", ");
            const cross = await query(`SELECT id FROM bond
         ...
         LIMIT 1`, [...bIds, ...aArr]);
           ...
           LIMIT 1`, [clusterIds[i], clusterIds[j]]);
...
// Main entry
// ---------------------------------------------------------------------------
export async function runClustering(userId) {
