...
export async function runEmergence(userId) {
    ...
    console.log(`[emergence] Done: +${result.higherOrderClusters} L2, ${result.absorbed} absorbed, ` +
    ...
    const structureChanged = result.absorbed + result.released + result.dissolved +
        result.higherOrderClusters + result.merged;
    if (structureChanged > 0) {
        try {
            // 查该用户所有 active L1 cluster → 回刷其成员 strike 的 source record
            const allL1 = await query(`SELECT id FROM strike
...
// ── 阶段 1: 吸纳 ───────────────────────────────────────────
async function phaseAbsorb(userId) {
    // 查所有 active L2 及其 L1 成员
    const l2s = await query(`SELECT * FROM strike
     WHERE user_id = $1 AND is_cluster = true AND level = 2
       AND status = 'active' AND embedding IS NOT NULL`, [userId]);
    if (l2s.length === 0)
        return 0;
    // 查所有自由 L1
    const freeL1s = await query(FREE_L1_SQL, [userId]);
    if (freeL1s.length === 0)
        return 0;
    ...
    for (const free of freeL1s) {
        ...
        for (const l2 of l2s) {
            ...
            const row = await queryOne(`SELECT AVG(1 - (f.embedding <=> m.embedding))::float AS avg_sim
         FROM strike f, strike m, bond b
         WHERE f.id = $1
           AND b.source_strike_id = $2 AND b.type = 'cluster_member'
           AND m.id = b.target_strike_id
           AND m.embedding IS NOT NULL
           AND f.embedding IS NOT NULL`, [free.id, l2.id]);
...
// ── 阶段 2: 释放 ───────────────────────────────────────────
async function phaseRelease(userId) {
    const l2s = await query(`SELECT * FROM strike
     WHERE user_id = $1 AND is_cluster = true AND level = 2
    ...
    for (const l2 of l2s) {
        // 查该 L2 的所有 L1 成员
        const members = await query(`SELECT s.id, s.nucleus FROM strike s
       ...
       WHERE b.source_strike_id = $1 AND b.type = 'cluster_member'
         ...
         AND s.embedding IS NOT NULL`, [l2.id]);
        if (members.length <= 1)
            continue; // 单成员 L2 不释放
        // 对每个成员，计算与其余成员的平均相似度
        for (const m of members) {
            ...
            if (otherIds.length === 0)
                continue;
            const row = await queryOne(`SELECT AVG(1 - (a.embedding <=> b.embedding))::float AS avg_sim
         FROM strike a, strike b
         WHERE a.id = $1 AND b.id = ANY($2)
           AND a.embedding IS NOT NULL AND b.embedding IS NOT NULL`, [m.id, otherIds]);
            const sim = row?.avg_sim ?? 1.0;
            if (sim < RELEASE_THRESHOLD) {
                // 删除 cluster_member bond
                await execute(`DELETE FROM bond
           WHERE source_strike_id = $1 AND target_strike_id = $2 AND type = 'cluster_member'`, [l2.id, m.id]);
                released++;
...
// ── 阶段 3: 清理 ───────────────────────────────────────────
async function phaseCleanup(userId) {
    // 查所有 active L2，检查是否还有成员
    const emptyL2s = await query(`SELECT s.id FROM strike s
    ...
    for (const l2 of emptyL2s) {
        ...
        await execute(`UPDATE todo SET cluster_id = NULL WHERE cluster_id = $1`, [l2.id]);
        console.log(`[emergence:cleanup] Dissolved empty L2 ${l2.id}`);
    }
    return emptyL2s.length;
}
// ── 阶段 4: 创建（原有逻辑） ───────────────────────────────
async function phaseCreate(userId) {
    ...
    const similarPairs = await query(`SELECT a.id AS id_a, b.id AS id_b,
            1 - (a.embedding <=> b.embedding) AS similarity
     FROM strike a, strike b
     WHERE a.id = ANY($1) AND b.id = ANY($1) AND a.id < b.id
       AND a.embedding IS NOT NULL AND b.embedding IS NOT NULL
       AND 1 - (a.embedding <=> b.embedding) >= $2`, [clusterIds, SIMILARITY_THRESHOLD]);
    // 连通分量
    const adj = new Map();
    for (const c of freeL1s)
    ...
    for (const cId of clusterIds) {
        ...
        while (stack.length > 0) {
            ...
            if (neighbors) {
                neighbors.forEach((n) => { if (!visited.has(n))
                    stack.push(n); });
            }
        }
...
// ── 阶段 5: L2 合并 ────────────────────────────────────────
async function phaseMergeL2(userId) {
    const l2s = await query(`SELECT * FROM strike
     WHERE user_id = $1 AND is_cluster = true AND level = 2
       AND status = 'active' AND embedding IS NOT NULL`, [userId]);
    if (l2s.length < 2)
    ...
    const pairs = await query(`SELECT a.id AS id_a, b.id AS id_b,
            1 - (a.embedding <=> b.embedding) AS similarity
     FROM strike a, strike b
     WHERE a.id = ANY($1) AND b.id = ANY($1) AND a.id < b.id
       AND a.embedding IS NOT NULL AND b.embedding IS NOT NULL
       AND 1 - (a.embedding <=> b.embedding) >= $2`, [l2Ids, L2_MERGE_THRESHOLD]);
    if (pairs.length === 0)
        return 0;
    ...
    for (const pair of pairs) {
        if (mergedIds.has(pair.id_a) || mergedIds.has(pair.id_b))
            continue;
...
- reason: 简短理由`,
            ...
            for (const oldL2Id of [pair.id_a, pair.id_b]) {
                const memberBonds = await query(`SELECT * FROM bond
           WHERE source_strike_id = $1 AND type = 'cluster_member'`, [oldL2Id]);
                if (memberBonds.length > 0) {
                    await bondRepo.createMany(memberBonds.map((b) => ({
...
// ── 阶段 6: Bond 继承 ──────────────────────────────────────
async function phaseBondInheritance(userId) {
    const l2Clusters = await query(`SELECT * FROM strike
     WHERE user_id = $1 AND is_cluster = true AND level = 2 AND status = 'active'
     ORDER BY created_at DESC LIMIT 20`, [userId]);
    if (l2Clusters.length < 2)
        return 0;
    let inherited = 0;
