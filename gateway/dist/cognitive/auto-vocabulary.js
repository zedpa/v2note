...
/** 自动收集词汇，返回新增词汇数 */
export async function autoCollectVocabulary(deviceId, userId) {
    try {
        // 1. 查询最近 7 天的 transcript 文本
        const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
        const rows = await query(`SELECT t.text FROM transcript t
       JOIN record r ON r.id = t.record_id
       WHERE r.device_id = $1 AND r.created_at >= $2 AND t.text IS NOT NULL`, [deviceId, sevenDaysAgo]);
        if (rows.length === 0)
            return 0;
        // 2. 提取词汇并统计频率
...
//# sourceMappingURL=auto-vocabulary.js.map