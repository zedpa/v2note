 ...
 */
export async function generateTopLevelDimensions(userId, answerText) {
 ...
 */
export async function matchToTopLevel(nucleus, topLevels) {
    ...
    for (const tl of topLevels) {
        ...
        if (!tlEmbedding || tlEmbedding.length === 0)
            continue;
