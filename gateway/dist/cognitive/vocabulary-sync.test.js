...
function mockFetchSuccess(vocabularyId) {
...
describe("syncVocabularyToDashScope", () => {
    ...
    it("should_create_vocabulary_when_no_vocabulary_id_exists", async () => {
        ...
        expect(mockExecute).toHaveBeenCalledWith(expect.stringContaining("app_user"), expect.arrayContaining(["vocab-new-123", "user-1"]));
    });
    it("should_update_vocabulary_when_vocabulary_id_exists", async () => {
        mockQuery.mockImplementation((sql) => {
            ...
            if (sql.includes("FROM app_user"))
                return [{ asr_vocabulary_id: "vocab-existing-456" }];
            return [];
        });
    ...
    it("should_share_vocabulary_id_across_devices_of_same_user", async () => {
        ...
        mockQuery.mockImplementation((sql) => {
            if (sql.includes("FROM device"))
                return [{ user_id: "user-1" }];
            if (sql.includes("FROM app_user"))
                return [{ asr_vocabulary_id: "vocab-shared-789" }];
            return [];
        });
    ...
    it("should_map_frequency_to_weight_correctly", async () => {
        ...
        const vocab = body.input.vocabulary;
    ...
    it("should_truncate_to_500_words_by_frequency", async () => {
        const entries = Array.from({ length: 600 }, (_, i) => makeEntry(`词${i}`, 600 - i));
...
//# sourceMappingURL=vocabulary-sync.test.js.map