...
export function shouldUpdateSoulStrict(userMessages) {
    return userMessages.some((msg) => SOUL_UPDATE_PATTERNS.some((pattern) => pattern.test(msg)));
}
/** 比对 Plan 的 original vs final steps，提取偏好 */
export function extractPlanPreference(originalSteps, finalSteps, similarCount) {
    if (similarCount < 3)
        return null;
    // 找出新增的步骤
...
export function classifyProfileFact(factContent) {
...
export async function findStalePreferences(userId, staleDays) {
    return query(`SELECT id, content, updated_at FROM memory
     WHERE user_id = $1
...
export async function decayPreferences(userId) {
    // 标记 60 天未更新的为 stale
    await execute(`UPDATE memory SET metadata = jsonb_set(COALESCE(metadata, '{}'), '{stale}', 'true')
     WHERE user_id = $1
       AND source = 'interaction'
       AND updated_at < NOW() - INTERVAL '60 days'
       AND (metadata->>'stale') IS DISTINCT FROM 'true'`, [userId]);
    // 删除 90 天未更新的
    await execute(`DELETE FROM memory
     WHERE user_id = $1
       AND source = 'interaction'
       AND updated_at < NOW() - INTERVAL '90 days'`, [userId]);
}
/** 聚合近 30 天的 unmet_request */
export async function aggregateUnmetRequests(userId) {
    const rows = await query(`SELECT request_text, COUNT(*)::text as count
     FROM unmet_request
     WHERE user_id = $1
