...
export async function create(fields) {
    await execute(`INSERT INTO memory (device_id, user_id, content, source_date, importance)
     VALUES ($1, $2, $3, $4, $5)`, [
        fields.device_id,
        fields.user_id ?? null,
        fields.content,
...
export async function deleteByIdAndUser(id, userId) {
    await execute(`DELETE FROM memory WHERE id = $1 AND user_id = $2`, [id, userId]);
}
...
export async function updateByUser(id, userId, fields) {
    ...
    await execute(`UPDATE memory SET ${sets.join(", ")} WHERE id = $${i++} AND user_id = $${i}`, params);
}
