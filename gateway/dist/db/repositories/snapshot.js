...
export async function findByUser(userId) {
    try {
        const row = await queryOne(`SELECT * FROM cognitive_snapshot WHERE user_id = $1`, [userId]);
        if (!row)
            return null;
        // 验证 JSONB 字段可解析（防止损坏）
...
export async function upsert(userId, data) {
    ...
    await execute(`INSERT INTO cognitive_snapshot (user_id, clusters, goals, contradictions, patterns, last_analyzed_strike_id, strike_count, version, updated_at)
...
// ── 查询新增 Strike 数量 ──────────────────────────────────────────────
export async function countNewStrikes(userId) {
    const snapshot = await queryOne(`SELECT last_analyzed_strike_id FROM cognitive_snapshot WHERE user_id = $1`, [userId]);
    ...
    const row = await queryOne(`SELECT COUNT(*)::text as count FROM strike
...
export async function getNewStrikes(userId, lastStrikeId, limit = 300) {
    if (!lastStrikeId) {
        ...
        return query(`SELECT s.id, s.nucleus, s.polarity, s.source_type, s.created_at,
       ...
       GROUP BY s.id
       ORDER BY s.created_at ASC LIMIT $2`, [userId, limit]);
    }
    return query(`SELECT s.id, s.nucleus, s.polarity, s.source_type, s.created_at,
            string_agg(st.label, ',') as tags
     FROM strike s
     ...
     GROUP BY s.id
     ORDER BY s.created_at ASC LIMIT $3`, [userId, lastStrikeId, limit]);
}
