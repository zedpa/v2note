...
import { create, findById, findByUser, update, updateStatus, findByParent, findRoots, } from "./wiki-page.js";
...
describe("wiki-page repository", () => {
    ...
    describe("create", () => {
        it("should_insert_wiki_page_when_basic_fields_provided", async () => {
            vi.mocked(queryOne).mockResolvedValue(mockPage);
        ...
        it("should_include_embedding_when_provided", async () => {
            vi.mocked(queryOne).mockResolvedValue({ ...mockPage, embedding: [0.1, 0.2] });
            await create({
                user_id: "u-1",
                title: "测试",
    ...
    describe("findById", () => {
        it("should_return_page_when_found", async () => {
            ...
            expect(queryOne).toHaveBeenCalledWith(expect.stringContaining("SELECT * FROM wiki_page WHERE id = $1"), ["wp-1"]);
        });
    ...
    describe("findByUser", () => {
        it("should_return_all_pages_for_user_when_no_status_filter", async () => {
            vi.mocked(query).mockResolvedValue([mockPage]);
    ...
    describe("updateStatus", () => {
        it("should_update_status_when_no_merged_into", async () => {
            ...
            expect(execute).toHaveBeenCalledWith(expect.stringContaining("status = $1"), ["archived", "wp-1"]);
            const sql = vi.mocked(execute).mock.calls[0][0];
            expect(sql).toContain("updated_at = now()");
    ...
    describe("findByParent", () => {
        it("should_return_active_children_of_parent", async () => {
            vi.mocked(query).mockResolvedValue([{ ...mockPage, parent_id: "wp-parent" }]);
...
//# sourceMappingURL=wiki-page.test.js.map