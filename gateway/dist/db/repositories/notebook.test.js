...
describe("notebook repository", () => {
    ...
    describe("findByDevice", () => {
        it("returns notebooks ordered by system flag", async () => {
            ...
            expect(query).toHaveBeenCalledWith(expect.stringContaining("ORDER BY is_system DESC"), ["dev-1"]);
        });
    });
    describe("findOrCreate", () => {
        it("creates new notebook on first call", async () => {
            ...
            expect(queryOne).toHaveBeenCalledWith(expect.stringContaining("ON CONFLICT"), ["dev-1", "project-alpha", "Alpha notes", false, "#6366f1"]);
        });
    ...
    describe("ensureSystemNotebooks", () => {
        it("creates ai-self and default notebooks", async () => {
            vi.mocked(queryOne).mockResolvedValue({ id: "nb-1", name: "system" });
