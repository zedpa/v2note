import { sendJson, sendError, getDeviceId, readBody } from "../lib/http-helpers.js";
...
export function registerNotebookRoutes(router) {
    ...
    router.post("/api/v1/notebooks", async (req, res) => {
        ...
        const nb = await notebookRepo.findOrCreate(deviceId, body.name.trim(), body.description?.trim(), false, body.color?.trim());
        sendJson(res, nb, 201);
    });
    ...
    router.get("/api/v1/diary/:notebook/:date", async (req, res, params) => {
        const deviceId = getDeviceId(req);
        const entry = await aiDiaryRepo.findFull(deviceId, params.notebook, params.date);
    ...
    router.get("/api/v1/diary/:notebook", async (req, res, params) => {
        ...
        const summaries = await aiDiaryRepo.findSummaries(deviceId, params.notebook, start, end);
