...
export function registerTopicRoutes(router) {
    ...
    router.get("/api/v1/topics", async (req, res) => {
        try {
            ...
            const clusters = await query(`SELECT * FROM strike WHERE user_id = $1 AND is_cluster = true AND status = 'active'
         ORDER BY created_at DESC`, [userId]);
            ...
            const memberCounts = await query(`SELECT target_strike_id, COUNT(*) as cnt FROM bond
         WHERE target_strike_id = ANY($1) AND type = 'cluster_member'
         GROUP BY target_strike_id`, [clusterIds]);
            const memberCountMap = new Map(memberCounts.map((r) => [r.target_strike_id, parseInt(r.cnt, 10)]));
            // 3. 批量查询关联的 active goals
            const activeGoals = await query(`SELECT * FROM goal WHERE cluster_id = ANY($1) AND status = 'active'`, [clusterIds]);
            ...
            const polarityStats = await query(`SELECT b.target_strike_id,
                COUNT(*) as total,
         ...
         GROUP BY b.target_strike_id`, [clusterIds]);
            const polarityMap = new Map(polarityStats.map((r) => [
                r.target_strike_id,
                { total: parseInt(r.total, 10), intend: parseInt(r.intend_count, 10) },
            ]));
            // 5. 批量查询最近活动时间
            const lastActivities = await query(`SELECT b.target_strike_id, MAX(s.created_at) as last_at
         FROM bond b
         JOIN strike s ON s.id = b.source_strike_id
         WHERE b.target_strike_id = ANY($1) AND b.type = 'cluster_member'
         GROUP BY b.target_strike_id`, [clusterIds]);
    ...
    router.get("/api/v1/topics/:clusterId/lifecycle", async (_req, res, params) => {
        try {
            ...
            // 获取关联的所有 goals
            const allGoals = await query(`SELECT * FROM goal WHERE cluster_id = $1`, [clusterId]);
           ...
           ORDER BY t.priority DESC, t.created_at ASC`, [activeGoalIds]);
            ...
            for (const goal of activeGoals) {
                const todos = await query(`SELECT * FROM todo WHERE goal_id = $1 ORDER BY created_at ASC`, [goal.id]);
            ...
            const goalLinkedStrikeIds = activeGoalIds.length > 0
                ? await query(`SELECT DISTINCT t.strike_id FROM todo t
             WHERE t.goal_id = ANY($1) AND t.strike_id IS NOT NULL`, [activeGoalIds]).then((rows) => new Set(rows.map((r) => r.strike_id)))
                : new Set();
            const seedStrikes = await query(`SELECT s.* FROM strike s
         ...
         WHERE b.target_strike_id = $1
           ...
           AND s.status = 'active'
         ORDER BY s.created_at DESC`, [clusterId]);
            ...
            for (const goal of completedGoals) {
                ...
                const reviewStrike = await queryOne(`SELECT s.* FROM strike s
           JOIN bond b ON b.source_strike_id = s.id
           WHERE b.target_strike_id = $1
             AND s.polarity = 'judge'
           ORDER BY s.created_at DESC LIMIT 1`, [clusterId]);
                harvest.push({
                    goal,
