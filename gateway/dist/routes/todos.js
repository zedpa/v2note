...
export function registerTodoRoutes(router) {
    ...
    router.post("/api/v1/todos", async (_req, res) => {
        ...
        if (body.reminder_before && body.reminder_before > 0 && body.scheduled_start) {
            reminder_at = new Date(new Date(body.scheduled_start).getTime() - body.reminder_before * 60000).toISOString();
        }
        const shared = {
    ...
    router.patch("/api/v1/todos/:id", async (req, res, params) => {
        ...
        if (body.reminder_before === null) {
            ...
            updateFields.reminder_at = new Date(new Date(body.scheduled_start).getTime() - body.reminder_before * 60000).toISOString();
        }
