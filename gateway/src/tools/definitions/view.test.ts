...
import {
  recordRepo, transcriptRepo, summaryRepo,
  todoRepo, goalRepo, memoryRepo,
  userProfileRepo, soulRepo, skillConfigRepo,
...
describe("view (unified)", () => {
  ...
  it("should_view_record_full_content", async () => {
    ...
    vi.mocked(transcriptRepo.findByRecordId).mockResolvedValue({ text: "日记内容" } as any);
    vi.mocked(summaryRepo.findByRecordId).mockResolvedValue({ title: "标题" } as any);
  ...
  it("should_reject_record_of_other_user", async () => {
    vi.mocked(recordRepo.findById).mockResolvedValue({ id: "rec-1", user_id: "other" } as any);
    const result = await viewTool.handler({ type: "record", id: "rec-1" }, CTX);
    expect(result.success).toBe(false);
  });
