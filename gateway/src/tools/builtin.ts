...
async function handleCreateTodo(
  ...
  const updates: any = {};
  if (args.scheduled_start !== undefined) updates.scheduled_start = String(args.scheduled_start || "");
  if (args.scheduled_end !== undefined) updates.scheduled_end = String(args.scheduled_end || "");
  ...
  if (updates.scheduled_start === "") updates.scheduled_start = null;
  if (updates.scheduled_end === "") updates.scheduled_end = null;
...
async function handleIngest(
  ...
  const sourceType = (args.source_type as "think" | "material") ?? "material";
  ...
  digestRecords([record.id], { deviceId, userId }).catch(
    (err) => console.error("[builtin-tool] ingest digest failed:", err),
  );
