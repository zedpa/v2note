...
const DASHSCOPE_VOCAB_URL =
  "https://dashscope.aliyuncs.com/api/v1/services/audio/asr/vocabulary";
...
function mapFrequencyToWeight(freq: number): number {
  if (freq >= 10) return 5;
  if (freq >= 5) return 4;
  if (freq >= 1) return 3;
  return 2; // freq == 0（预设词）
...
async function getUserIdByDevice(deviceId: string): Promise<string | null> {
...
async function getUserVocabularyId(userId: string): Promise<string | null> {
  const rows = await query<{ asr_vocabulary_id: string | null }>(
    `SELECT asr_vocabulary_id FROM app_user WHERE id = $1`,
    [userId],
  );
...
async function saveUserVocabularyId(userId: string, vocabularyId: string): Promise<void> {
  await execute(
    `UPDATE app_user SET asr_vocabulary_id = $1 WHERE id = $2`,
    [vocabularyId, userId],
  );
}
...
export async function syncVocabularyToDashScope(deviceId: string): Promise<string> {
  try {
    ...
    const vocabulary: DashScopeWord[] = top500.map((e) => ({
      text: e.term,
 ...
 * 不触发同步，仅读取缓存值
 */
export async function getVocabularyIdForDevice(deviceId: string): Promise<string> {
  try {
    const rows = await query<{ asr_vocabulary_id: string | null }>(
      `SELECT u.asr_vocabulary_id
       FROM device d
