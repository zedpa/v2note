...
export interface DecisionContext {
  ...
  /** Related clusters */
  clusters: Array<{ id: string; name: string; memberCount: number }>;
...
export async function gatherDecisionContext(
  ...
  // 2. Related clusters
  try {
    const clusters = await query<StrikeEntry & { member_count: string }>(
      `SELECT s.*, COUNT(cm.target_strike_id) as member_count
       FROM strike s
  ...
  try {
    const patterns = await query<StrikeEntry>(
      `SELECT * FROM strike
