...
function classifyDuration(text: string): "quick" | "medium" | "deep" {
  if (QUICK_RE.test(text)) return "quick";
...
export async function computeActionPanel(userId: string): Promise<ActionPanel> {
  ...
  for (const goal of goalStrikes) {
    ...
    const linkedIds = bonds.map((b) =>
      b.source_strike_id === goal.id ? b.target_strike_id : b.source_strike_id,
    );

    if (linkedIds.length > 0) {
      const placeholders = linkedIds.map((_, i) => `$${i + 1}`).join(", ");
      const linkedStrikes = await query<StrikeEntry>(
        `SELECT * FROM strike WHERE id IN (${placeholders})
         AND polarity = 'intend' AND status = 'active'`,
        linkedIds,
      );

      for (const s of linkedStrikes) {
        if (goalStrikeIds.has(s.id)) continue; // skip goal-level strikes
        allActions.push({
          strikeId: s.id,
          text: s.nucleus,
          goalId: goal.id,
    ...
    const todos = await query<Todo>(
      `SELECT * FROM todo WHERE goal_id = $1 AND done = false ORDER BY priority DESC`,
      [goal.id],
    );
  ...
  const today: ActionItem[] = allActions.slice(1, 5).map((a) => {
  ...
  const goals: GoalIndicator[] = goalStrikes.map((g) => ({
    goalId: g.id,
    goalName: g.nucleus,
