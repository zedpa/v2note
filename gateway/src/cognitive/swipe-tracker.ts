...
export async function recordSwipe(event: SwipeEvent): Promise<void> {
...
async function handleRightSwipe(userId: string, strikeId: string): Promise<void> {
  ...
  const todo = await query<Todo>(
    `SELECT * FROM todo WHERE id = $1 AND done = false LIMIT 1`,
    [strikeId],
  );
...
async function handleLeftSwipe(
  userId: string,
  strikeId: string,
  reason?: "later" | "wait" | "blocked" | "rethink",
): Promise<void> {
