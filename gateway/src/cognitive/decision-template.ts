 ...
 */
export async function detectClosedLoops(userId: string): Promise<ClosedLoop[]> {
  ...
  const existingTemplates = await query<{ id: string; goal_id: string }>(
    `SELECT id, goal_id FROM decision_template WHERE goal_id = ANY($1)`,
    [goalIds],
  );
...
export async function saveTemplate(input: TemplateInput): Promise<string> {
  const row = await queryOne<{ id: string }>(
    `INSERT INTO decision_template (user_id, goal_id, title, steps, outcome, tags)
 ...
 * 通过关键词匹配查找相似的决策模板。
 */
export async function matchTemplate(userId: string, text: string): Promise<MatchedTemplate[]> {
  // 简单方案：全文搜索 title 匹配
  // 未来可用 embedding 提升精度
  const keywords = text
    .replace(/[，。！？、；：""''（）\s]/g, "")
  ...
  const templates = await query<{
    ...
    `SELECT dt.id, dt.title, dt.steps::text, dt.outcome
     FROM decision_template dt
     WHERE dt.user_id = $1
       AND (${likeConditions || "false"})
     ORDER BY dt.created_at DESC
     LIMIT 3`,
    [userId, ...keywords.slice(0, 5)],
  );
