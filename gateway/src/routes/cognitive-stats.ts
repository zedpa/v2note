...
export function registerCognitiveStatsRoutes(router: Router) {
  ...
  router.get("/api/v1/cognitive/stats", async (req, res) => {
    ...
    const [
      ...
      queryOne<CountRow>(
        `SELECT COUNT(*) as count FROM record WHERE user_id = $1`,
        [userId],
      ),
      queryOne<CountRow>(
        `SELECT COUNT(*) as count FROM record WHERE user_id = $1 AND compile_status = 'compiled'`,
        [userId],
      ),
      queryOne<CountRow>(
        `SELECT COUNT(*) as count FROM wiki_page WHERE user_id = $1`,
        [userId],
      ),
      queryOne<CountRow>(
        `SELECT COUNT(*) as count FROM wiki_page WHERE user_id = $1 AND status = 'active'`,
        [userId],
      ),
    ]);
