...
interface WikiLifecycleSeed {
  id: string;
  content: string;
  type: "section";
}

interface WikiLifecycleHarvest {
...
interface LifecycleResponse {
  ...
  seeds: WikiLifecycleSeed[];
  harvest: WikiLifecycleHarvest[];
}
  ...
  router.get("/api/v1/topics", async (req, res) => {
    try {
      ...
      const recordCounts = await query<{ wiki_page_id: string; cnt: string }>(
        `SELECT wiki_page_id, COUNT(*) as cnt FROM wiki_page_record
         WHERE wiki_page_id = ANY($1)
         GROUP BY wiki_page_id`,
        [pageIds],
      );
      const recordCountMap = new Map(
        recordCounts.map((r) => [r.wiki_page_id, parseInt(r.cnt, 10)]),
      );
      ...
      const goalsByPage = new Map<string, { id: string; title: string }[]>();
      for (const g of activeGoals) {
        if (!g.wiki_page_id) continue;
      ...
      // 4. 组装结果
      const topics: TopicItem[] = wikiPages.map((page) => {
  ...
  router.get("/api/v1/topics/:id/lifecycle", async (_req, res, params) => {
    try {
      ...
      const activeGoals = allGoals.filter(
        (g) => g.status === "active" || g.status === "progressing",
      );
      ...
      if (activeGoalIds.length > 0) {
        ...
        const allTodos = await query<{
          ...
          `SELECT parent_id, id, text, done FROM todo
           WHERE parent_id = ANY($1) AND level = 0
           ORDER BY created_at`,
          [activeGoalIds],
        );
        const todosByGoal = new Map<string, typeof allTodos>();
      ...
      const harvest: WikiLifecycleHarvest[] = completedGoals.map((goal) => {
        // 尝试匹配收获段落中与 goal 相关的内容
        const matchedParagraph = harvestParagraphs.find(
          (p) => p.content.includes(goal.title),
        );
        return {
