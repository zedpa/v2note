...
export function registerCompanionRoutes(router: Router) {
  ...
  router.get("/api/v1/companion/status", async (req, res) => {
    try {
      ...
      const [
        ...
        query<{ cnt: string }>(
          `SELECT COUNT(*) as cnt FROM todo t
           JOIN record r ON r.id = t.record_id
           WHERE r.device_id = $1 AND t.done = false AND t.skip_count >= 5`,
          [deviceId],
        ),
        ...
        query<{ cnt: string }>(
          `SELECT COUNT(*) as cnt FROM strike s
        ...
        query<{ cnt: string }>(
          `SELECT COUNT(*) as cnt FROM strike
        ...
        query<{ cnt: string }>(
          `SELECT COUNT(*) as cnt FROM record
           WHERE device_id = $1 AND created_at > NOW() - INTERVAL '48 hours'`,
          [deviceId],
        ),
      ]);
