...
export function registerNotebookRoutes(router: Router) {
  ...
  router.get("/api/v1/notebooks", async (req, res) => {
    ...
    await notebookRepo.ensureSystemNotebooksByUser(userId, userId);
  ...
  router.post("/api/v1/notebooks", async (req, res) => {
    ...
    const nb = await notebookRepo.findOrCreateByUser(userId, userId, body.name.trim(), body.description?.trim(), false, body.color?.trim());
    sendJson(res, nb, 201);
  });
  ...
  router.get("/api/v1/diary/:notebook/:date", async (req, res, params) => {
    const userId = getUserId(req);
    if (!userId) { sendError(res, "Unauthorized", 401); return; }
    const entry = await aiDiaryRepo.findFullByUser(userId, params.notebook, params.date);
    sendJson(res, entry ?? { device_id: undefined, notebook: params.notebook, entry_date: params.date, summary: "", full_content: "" });
  });
  ...
  router.get("/api/v1/diary/:notebook", async (req, res, params) => {
    ...
    const summaries = await aiDiaryRepo.findSummariesByUser(userId, params.notebook, start, end);
