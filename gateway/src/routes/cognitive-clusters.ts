          ...
          const [{ count }] = await query<{ count: string }>(
            `SELECT COUNT(*) as count FROM bond
             WHERE type = 'contradiction'
               AND source_strike_id = ANY($1) AND target_strike_id = ANY($1)`,
            [ids],
          );
    ...
    const contradictions =
      memberIds.length > 0
        ? await query<{
            ...
            `SELECT sa.id as sa_id, sa.nucleus as sa_nucleus,
             ...
             WHERE b.type = 'contradiction'
               AND b.source_strike_id = ANY($1) AND b.target_strike_id = ANY($1)`,
            [memberIds],
          )
        : [];

    // Patterns (realize + inference source_type)
    const patterns = members.filter(
      (m) => m.polarity === "realize",
    );
