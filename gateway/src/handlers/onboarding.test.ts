...
vi.mock("../db/repositories/index.js", () => ({
  ...
  transcriptRepo: {
    create: (...args: any[]) => mockTranscriptCreate(...args),
  },
}));
...
vi.mock("./process.js", () => ({
  processEntry: (...args: any[]) => mockProcessEntry(...args),
}));
...
describe("Onboarding v3: 两步引导", () => {
  ...
  it("should_save_name_when_step_1", async () => {
    ...
    expect(mockUpsertOnboardingField).toHaveBeenCalledWith(
      "user-1", "name", "小明", "dev-1",
    );
  });
  ...
  it("should_process_thought_and_return_result_when_step_2", async () => {
    ...
    expect(mockRecordCreate).toHaveBeenCalledWith(
      expect.objectContaining({ user_id: "user-1", source: "manual" }),
    );
    ...
    expect(mockUpsertOnboardingField).toHaveBeenCalledWith(
      "user-1", "onboarding_done", "true", "dev-1",
    );
  });
