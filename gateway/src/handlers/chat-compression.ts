...
import {
  ...
  markCompressed,
  getContextSummaries,
} from "../db/repositories/chat-message.js";
 ...
 */
export async function compressMessages(userId: string): Promise<void> {
  // 取所有未压缩消息（按时间正序，最早在前）
  ...
  console.log(
    `[compression] Compressed ${toCompress.length} messages for user ${userId}, summary ${summary.length} chars`,
  );
}
...
async function mergeSummaries(
  userId: string,
  summaries: Awaited<ReturnType<typeof getContextSummaries>>,
): Promise<void> {
    ...
    [
      {
        role: "system",
        content:
          "请将以下多段对话摘要合并为一段连贯的摘要。保留所有关键信息，去除重复内容。",
      },
  ...
  console.log(
    `[compression] Merged ${summaries.length} summaries into 1 (${merged.length} chars)`,
  );
}
 ...
 * 不阻塞当前请求
 */
export async function maybeCompress(userId: string): Promise<void> {
  try {
