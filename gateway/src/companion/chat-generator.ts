...
export type ChatTrigger =
  | "daily_open"       // 每日首次打开
  | "todo_complete"    // 完成待办
  | "digest_insight"   // Digest 发现关联
  | "idle_prompt";     // 停留无操作

interface ChatContext {
  mood: string;
  recentStrikes: Array<{ nucleus: string; polarity: string }>;
  timeOfDay: "morning" | "afternoon" | "evening" | "night";
  userProfile?: { occupation?: string; interests?: string[] };
}
...
/** 获取随机奖励语（不消耗 AI 调用量） */
export function getRewardPhrase(): string {
  return REWARD_PHRASES[Math.floor(Math.random() * REWARD_PHRASES.length)];
}

/** AI 生成主动闲聊（一句话，≤30字） */
export async function generateCompanionChat(
  trigger: ChatTrigger,
  context: ChatContext,
): Promise<string> {
...
只输出这一句话，不要其他内容。`;
  ...
  try {
    ...
    // AI 调用失败，返回默认问候
    const defaults: Record<ChatTrigger, string> = {
      daily_open: "新的一天，加油",
...
export class ChatRateLimiter {
  ...
  private dailyCounts = new Map<string, number>(); // date → count
  ...
  canSend(): boolean {
    ...
    // 两条间隔 ≥ 2 小时
    if (now - this.lastChatTime < 2 * 60 * 60 * 1000) return false;
  ...
  record(): void {
    ...
    for (const [date] of this.dailyCounts) {
      if (date < today) this.dailyCounts.delete(date);
    }
  }
}
