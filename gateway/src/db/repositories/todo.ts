...
export async function update(
  ...
  // 通用字段遍历，减少重复代码
  const entries: Array<[string, any, ((v: any) => any)?]> = [
    ["text", fields.text],
    ["done", fields.done],
    ["estimated_minutes", fields.estimated_minutes],
    ["scheduled_start", fields.scheduled_start],
...
export async function dedupCreate(fields: {
  ...
  // 查询用户未完成的 level=0 待办
  const existing = await query<Todo>(
    `SELECT * FROM todo WHERE user_id = $1 AND done = false AND (level = 0 OR level IS NULL)`,
    [userId],
  );
...
/** 检查某日某模板是否已有实例 */
export async function hasInstanceForDate(
  templateId: string,
  date: string,
): Promise<boolean> {
...
/** 标记提醒已发送 */
export async function markReminderSent(todoId: string): Promise<void> {
  await execute(
    `UPDATE todo SET reminder_sent = true WHERE id = $1`,
    [todoId],
  );
}

/** 根据 scheduled_start 和 reminder_before 重算 reminder_at */
export async function recalcReminderAt(todoId: string): Promise<void> {
  await execute(
    `UPDATE todo SET
       reminder_at = CASE
