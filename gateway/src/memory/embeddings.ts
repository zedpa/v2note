...
export async function semanticSearch(
  query: string,
  memories: Array<{ id: string; content: string; source_date: string | null; importance: number }>,
  limit: number = 10,
  ...
  const BATCH_SIZE = 10;
  const memoryEmbeddings: Array<{ memory: typeof memories[0]; embedding: number[] }> = [];
...
export async function findSimilarMemory(
  newContent: string,
  existingMemories: Array<{ id: string; content: string; source_date: string | null; importance: number }>,
