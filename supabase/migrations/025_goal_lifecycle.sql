...
CREATE INDEX IF NOT EXISTS idx_action_event_todo ON action_event(todo_id);
CREATE INDEX IF NOT EXISTS idx_action_event_type ON action_event(type, created_at);

-- todo 增加 skip_count 便于快速查询
ALTER TABLE todo ADD COLUMN IF NOT EXISTS skip_count INTEGER NOT NULL DEFAULT 0;
