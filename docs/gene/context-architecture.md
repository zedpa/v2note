...
| **Warm** | Soul（仅 chat/briefing）、相关记忆（相关性评分 top N）、匹配技能全文、标签规则、输出格式 | ~2000-4000 chars | 异步，per-request |
...
### 技能分区策略
- `always: true` 技能始终注入全文（todo-extract, relay-detect）
...
- `gateway/src/context/loader.ts` — loadWarmContext()、rankMemories()、computeRelevanceScore()、extractKeywords()
