...
用户自己打字/语音产生的内容标记为 **Think**（原声/思考）——这是用户自己的认知，拥有完整权重。

用户可以在日记卡片上**点击标签一键切换**，不弹窗不确认。

```
record.source_type = 'think' | 'material'
...
- Think：完整认知权重，参与 Cluster 涌现、目标涌现、每日回顾
- 素材：降级权重（1/5~1/10），不参与涌现，只被吸附到已有 Cluster
...
- **🎙 语音按钮**（右侧）：点击切换到**独立录音界面**（复用现有 RecordingImmersive 沉浸式录音）。录音完成后转写文本自动回填到输入框，用户可以编辑后发送
...
参数：
  ...
  file?: File               // 图片/文件
  source_type: 'think' | 'material'   // 默认 material，用户打字/语音='think'
  metadata?: {
...
POST /ingest
  ...
  └─ type=audio
      ...
      → ASR 转写
      → Process（仅文本优化）
      → 创建 record(source_type='think') + transcript
      → 触发 Digest
```
