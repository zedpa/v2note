...
```
设计 AI 伴侣消息气泡，参考 Apple iMessage 的气泡但更具品牌感：

位置：日记流或待办流的最顶部（固定区域，两种视图共享）

基础气泡样式：
- 左侧 🦌 路路图标（24pt）
- 消息文字，body 字体
- 背景：primary/5%（极浅 deer 色）
- 边框：primary/10%
- 圆角 12pt
- 入场动画：translateY(8→0)，300ms ease-out

五种气泡变体（按优先级，高覆盖低）：

1. action.confirm（指令确认 — 高风险操作）：
   - 左侧晨光色竖线 3px (#E8A87C)
   - 确认文字（如「确认取消"周五评审会"吗？」）
   - [确认][算了] 两个 Pebble 按钮
   - 参考 Apple 通知中的 action buttons
   - 不自动消失，30s 超时后显示「已超时取消」
   - 触发场景：用户通过语音/文字发出删除、批量修改等高风险指令

2. action.result（执行结果 — 语音指令反馈）：
   - 左侧森林色竖线 3px (#5C7A5E)
   - ✅ 前缀 + 结果文字（如「已将'打给张总'改到明天下午3点」）
   - 右下「查看 →」antler 色链接（跳转对应视图）
   - 5s 后自动淡出降级为 ai.status
   - 参考 Apple 的 inline completion feedback
   - 触发场景：用户通过语音/文字发出修改、完成、创建等指令后

3. reflect.question（AI 追问）：
   - 标准气泡，点击后打开参谋对话 overlay（mode=review）
   - AI 追问作为 assistant 消息传入对话
   - 参考 iMessage 的 tap-to-expand

4. proactive.*（主动推送）：
   - 晨间简报 / 待办提醒 / 转达提醒等
   - 类似 Apple Siri Suggestions 的卡片

5. ai.status（AI 状态）：
   - 弱文字色小字，如「路路正在打盹」
   - 类似 Apple iMessage 的 typing indicator
   - 无消息时的默认态
```
...
第三组 — 目标管理（参考 Apple Reminders 的列表管理）：
  「我的目标」Serif 小标题 + [+] 新建按钮
  📁 项目名 + 活跃目标数 → 点击打开项目详情 overlay
  🎯 独立目标名 + 进度 → 点击打开目标详情 overlay
  💡 路路建议 +「确认?」标签 → AI 涌现建议（Cluster 意图密度 > 30% 触发）
  目标状态流转：suggested → active → archived
...
```
设计 AI 对话界面，参考 Apple Messages 的对话布局 + Siri 的智能感：
...
从 AI 追问气泡进入时，追问作为 assistant 消息预填（不是用户消息）
```
