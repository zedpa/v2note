...
for spec_file in "$SPECS_DIR"/*.md; do
  ...
  test_file="$TESTS_DIR/$filename.test.ts"
  ...
  if [ -f "$test_file" ]; then
    ...
    if [ "$test_count" -ge "$scenario_count" ]; then
      echo -e "  ${GREEN}✅${NC} $filename  ${status:-?}  场景:$scenario_count  测试组:$test_count"
    else
      echo -e "  ${YELLOW}⚠️${NC}  $filename  ${status:-?}  场景:$scenario_count  测试组:$test_count (覆盖不足)"
    fi
  else
    MISSING=$((MISSING + 1))
